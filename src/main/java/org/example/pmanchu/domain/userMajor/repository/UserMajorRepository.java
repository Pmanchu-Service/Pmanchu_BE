package org.example.pmanchu.domain.userMajor.repository;

import org.example.pmanchu.domain.notification.domain.Notification;
import org.example.pmanchu.domain.user.domain.User;
import org.example.pmanchu.domain.userMajor.domain.Specialization;
import org.example.pmanchu.domain.userMajor.domain.UserMajor;
import org.springframework.data.repository.CrudRepository;

import java.util.List;

public interface UserMajorRepository extends CrudRepository<UserMajor, Long> {
    void deleteByUserId(User userId);
    List<UserMajor> findAllByUserId(User userId);
    List<UserMajor> findAllUserBySpecializationIn(List<Specialization> specialization);
}
